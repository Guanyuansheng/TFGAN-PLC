...
class darcn(nn.Module):
    def __init__(self, causal_flag, stage_number):
        ...
        self.stft = ConvSTFT(win_len=320, win_inc=160, fft_len=320, win_type='hanning', feature_type='complex', fix=True)
        self.istft = ConviSTFT(win_len=320, win_inc=160, fft_len=320, win_type='hanning', feature_type='complex', fix=True)
    ...
    def forward(self, input):
        specs = self.stft(input)
        real = specs[:, :320 // 2 + 1]
        imag = specs[:, 320 // 2 + 1:]
        x = torch.sqrt(real ** 2 + imag ** 2)
...
class MEncoder(nn.Module):
    def __init__(self):
        ...
        self.pad4 = nn.ConstantPad2d((1, 0, 0, 0), value=0.)
...
if __name__ == '__main__':
    ...
    in_wav = torch.randn(3, 1, 3200)  # (B, channels, T).
    # in_spec = torch.randn(3, 322, 43)

    print(in_wav.shape)

    out_list = model(in_wav)
    print(out_list[1].size())

    # model params
    pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(
        'Total params: {} ({:.2f} MB)'.format(pytorch_total_params, (float(pytorch_total_params) * 4) / (1024 * 1024)))